...
class AlgHybridGMB : public Algorithm<Adapter>
{
  ...
  public:
    ...
    void hybridGMB(const size_t nVtx,lno_t nInterior, Teuchos::ArrayView<const lno_t> adjs, 
      ...
      recoloringSize(0) = 0;
      //an atomic copy, no longer used -> TODO remove this.
	...
	send_flags(i) = false;	      
      ...
      //TODO is the following variable used?
      ...
      this->colorInterior<execution_space, memory_space,memory_space>
                 (nVtx, dist_adjs, dist_offsets, colors, femv,dist_adjs,0,use_vbbit);
	...
	//communicate
        gno_t recv, sent;
        comm_time = doOwnedToGhosts(mapOwnedPlusGhosts,nVtx,verts_to_send_view,verts_to_send_size, send_flags,femv_colors,recv,sent);
        sentPerRound[0] = sent;
        recvPerRound[0] = recv;
	...
	//detect conflicts on the device, uncolor conflicts consistently.
        ...
        Kokkos::fence();
	//build the verts to send list.TODO are send_flags used anymore?
        Kokkos::parallel_for(nVtx, KOKKOS_LAMBDA(const int& i){
          if(femv_colors(i) == 0){
            verts_to_send_atomic(verts_to_send_size_atomic(0)++) = i;
	    send_flags(i) = true;
      ...
      while(recoloringSize(0) > 0 || !done){
        ...
        if(verbose) std::cout<<comm->getRank()<<": max_uncolored_degree = "<<max_uncolored_degree<<"\n";
	...
	if(verts_to_send_size(0) > 0){
        ...
        double comm_temp = timer();
	//gno_t sent,recv;
        commPerRound[distributedRounds] = doOwnedToGhosts(mapOwnedPlusGhosts,nVtx,verts_to_send_host, verts_to_send_size,send_flags,femv_colors,recv,sent); 	
        if(verbose)std::cout<<comm->getRank()<<": total sent in round "<<distributedRounds<<" = "<<sent<<"\n";
        if(verbose)std::cout<<comm->getRank()<<": total recv in round "<<distributedRounds<<" = "<<recv<<"\n";
