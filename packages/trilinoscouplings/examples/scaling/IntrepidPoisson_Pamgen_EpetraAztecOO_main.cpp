  ...
  *out << "||B - A*X_exact||_2 = " << norms[0] << endl
  ...
  RCP<operator_type> M;
  {
    ...
    if (prec_type == "ML") {
...
#else
      RCP<Xpetra::CrsMatrix<ST, Xpetra::EpetraNode> > mueluA_ = rcp(new Xpetra::EpetraCrsMatrixT<int, Xpetra::EpetraNode>(A));
      RCP<Xpetra::Matrix <ST, Xpetra::EpetraNode> > mueluA  = rcp(new Xpetra::CrsMatrixWrap<ST, Xpetra::EpetraNode>(mueluA_));
#endif
      ...
      RCP<MueLu::Hierarchy<ST, Xpetra::EpetraNode> > H = mueLuFactory.CreateHierarchy();
#endif
